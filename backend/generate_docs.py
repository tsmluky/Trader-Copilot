...
def generate_endpoints_md():
    ...
    files_content = ["# API Endpoints Specification", "", "| Method | Path | Auth | Description |", "|---|---|---|---|"]
    ...
    for path in sorted(paths.keys()):
        ...
        for method, details in methods.items():
            ...
            if path.startswith("/admin") or path.startswith("/advisor") or path.startswith("/strategies"):
                 security = "**Yes**"
            elif path.startswith("/auth/users/me"):
                 security = "**Yes**"
            
            files_content.append(f"| {method_str} | `{path}` | {security} | {summary} |")
