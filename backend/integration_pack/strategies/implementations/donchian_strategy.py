...
from backend.core.trading_rules.side_generators import side_donchian
    ...
    def run(self, config: StrategyConfigInRuntime, market_data_service) -> List[Signal]:
        ...
        for token in config.tokens:
            ...
            df = market_data_service.get_ohlcv(
                token=token, timeframe=self.default_timeframe, limit=limit
            )
            ...
            side_series = side_donchian(
                df, n=params.get("n", 20), atr_pct_min=params.get("atr_pct_min", 0.005)
            )
