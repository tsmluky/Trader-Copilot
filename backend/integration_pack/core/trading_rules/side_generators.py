...
def side_ma_cross(df: pd.DataFrame, fast: int, slow: int) -> pd.Series:
    ...
    return pd.Series(
        np.where(cross_up, "LONG", np.where(cross_dn, "SHORT", "")), index=df.index
    )
...
def side_donchian(df: pd.DataFrame, n: int, atr_pct_min: float) -> pd.Series:
    ...
    return pd.Series(
        np.where(long_sig, "LONG", np.where(short_sig, "SHORT", "")), index=df.index
    )


def side_bbmr(
    df: pd.DataFrame, bb_period: int, bb_std: float, regime_thr: float
) -> pd.Series:
    """
