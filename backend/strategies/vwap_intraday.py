...
class VWAPIntradayStrategy(Strategy):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.vwap_bands_std = self.config.get(
            "vwap_bands_std", 1.0
        )  # Desviación estándar para bandas
        ...
        self.tp_pct = self.config.get(
            "tp_pct", 0.015
        )  # 1.5% TP (intraday más conservador)
    ...
    def analyze(self, df: pd.DataFrame, token: str, timeframe: str) -> List[Signal]:
        ...
        if touched_vwap_from_below and closed_above_vwap and high_volume:
            ...
            rationale = (
                f"VWAP Bounce (Volume {volume_ratio:.1f}x avg, Price reclaimed VWAP)"
            )

            signal = Signal(
                timestamp=(
                    d.index[-1]
                    if isinstance(d.index[-1], datetime)
