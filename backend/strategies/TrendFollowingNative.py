...
class TrendFollowingNative(Strategy):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.risk_reward = self.config.get(
            "risk_reward", 2.0
        )  # TP = 2 * Risk (Standard 2:1)
    ...
    def analyze(self, df: pd.DataFrame, token: str, timeframe: str) -> List[Signal]:
        ...
        adx_col = (
            f"ADX_{self.adx_period}"  # pandas_ta usually outputs ADX_14, DMP_14, DMN_14
        )
        ...
        if (prev_fast <= prev_slow) and (curr_fast > curr_slow):
            ...
            if adx > self.adx_threshold:
                ...
                signal = Signal(
                    timestamp=(
                        curr.name
                        if isinstance(curr.name, datetime)
