    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            with open("backend/pnl_dump.txt", "w", encoding="utf-8") as f:
                f.write(
                    f"{'ID':<6} | {'TOKEN':<6} | {'SOURCE':<20} | {'PnL':<10} | {'USER':<6} | {'TIMESTAMP'}\n"
                )
                f.write("-" * 90 + "\n")
                ...
                for r in rows:
                    ...
                    f.write(
                        f"{s_id:<6} | {token:<6} | {source:<20} | {pnl:<10.4f} | {user_id!s:<6} | {ts} {marker}\n"
                    )
                    total_pnl += pnl
                ...
                if not found_suspect:
                    f.write(
                        "No single row with 0.29 found. Searching for combinations...\n"
                    )
