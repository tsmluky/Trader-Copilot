...
def rank_strategies():
    ...
    # 2. ROI positivo
    qualified = df[(df["total_trades"] >= 10) & (df["roi_pct"] > 0)].copy()
    ...
    print("=" * 80)
    print(
        f"{'Estrategia':<20} | {'Token':<6} | {'TF':<4} | {'ROI %':<8} | {'Win Rate':<8} | {'Trades':<6}"
    )
    print("-" * 80)

    for _, row in top_performers.iterrows():
        print(
            f"{row['strategy']:<20} | {row['token']:<6} | {row['timeframe']:<4} | {row['roi_pct']:>6.2f}% | {row['win_rate']:>6.1f}% | {row['total_trades']:<6}"
        )
