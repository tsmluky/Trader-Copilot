...
async def run_native_benchmark():
    ...
    for token in tokens:
        for tf in timeframes:
            ...
            try:
                # 1. Fetch Data
                raw_data = get_ohlcv_data(
                    token, tf, limit=1500
                )  # Approx 2 months for 1h, plenty for others
                if not raw_data:
                ...
                for i in range(len(df)):
                    ...
                    if not position:
                        relevant_sig = next(
                            (s for s in signals if s.timestamp == current_time), None
                        )
                        if relevant_sig:
                ...
                results.append(res)
                print(
                    f"  -> ROI: {res['roi']}% | Trades: {trades} | WR: {res['win_rate']}%"
                )
    ...
    for r in results:
        print(
            f"{r['token']:<10} {r['tf']:<5} {r['trades']:<8} {r['win_rate']:<6} {r['roi']:<8}"
        )
    print("=" * 50)
