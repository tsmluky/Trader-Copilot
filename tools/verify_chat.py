...
def verify_chat():
    ...
    chat_resp = requests.post(
        f"{BASE_URL}/advisor/chat", json=chat_payload, headers=headers
    )
    ...
    if chat_resp.status_code == 200:
        ...
        chat_resp = requests.post(
            f"{BASE_URL}/copilot/chat", json=chat_payload, headers=headers
        )
        print(
            f"[{'+' if chat_resp.status_code == 200 else '!'}] Alternate Response: {chat_resp.status_code} - {chat_resp.text}"
        )
    else:
