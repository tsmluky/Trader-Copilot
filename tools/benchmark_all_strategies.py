...
def run_benchmark():
    ...
    if files:
        ...
        print("resume_mode: ON (Skipping entries marked SUCCESS)")
    else:
    ...
    print(
        f"Trend Set: {len(TREND_STRATEGIES)} strats x {len(TREND_TOKENS)} tokens x {len(TREND_TIMEFRAMES)} TFs"
    )
    print(
        f"Reversion Set: {len(REVERSION_STRATEGIES)} strats x {len(REVERSION_TOKENS)} tokens x {len(REVERSION_TIMEFRAMES)} TFs"
    )
    ...
    trend_tests = (
        ...
        * len(TREND_TOKENS)
        * len(TREND_TIMEFRAMES)
        * len(DURATIONS)
    )
    rev_tests = (
        len(REVERSION_STRATEGIES)
        * len(REVERSION_TOKENS)
        * len(REVERSION_TIMEFRAMES)
        * len(DURATIONS)
    )
    ...
    print("💾 Each test saves immediately (crash-safe!)\n")
            ...
            for token in tokens:
                for timeframe in tfs:
                    for days in DURATIONS:
                        ...
                        print(
                            f"[{completed}/{total_tests}] {strategy} | {token} | {timeframe} | {days}d",
                            end=" ... ",
                        )
                        ...
                        try:
                            ...
                            result_row = {
                                ...
                                "alpha": round(
                                    metrics["total_pnl"] - metrics["buy_hold_pnl"], 2
                                ),
                            ...
                            print(
                                f"{pnl_color} ${metrics['total_pnl']:.0f} | {metrics['total_trades']} trades | {elapsed:.1f}s"
                            )
...
def print_summary(results: List[Dict]):
    ...
    if success_results:
        sorted_by_pnl = sorted(
            success_results, key=lambda x: x["total_pnl"], reverse=True
        )[:5]
        ...
        for i, r in enumerate(sorted_by_pnl, 1):
            print(
                f"{i:<5} {r['strategy']:<20} {r['token']:<6} {r['timeframe']:<5} {r['days']:<6} "
        ...
        for token in all_tokens:
            ...
            if token_results:
                ...
                print(
                    f"  {token}: {best['strategy']} ({best['timeframe']}, {best['days']}d) → ${best['total_pnl']:.2f}"
                )
